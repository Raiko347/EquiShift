...
from PyQt5.QtWidgets import (
    ...
    QMessageBox,
    QDialog,
    QAbstractItemView # NEU: Wichtig für Mehrfachauswahl
)
...
class ShiftDetailsWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        # NEU: Mehrfachauswahl erlauben (Strg+Klick / Shift+Klick)
    ...
    def _add_helper(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            # Falls der Dialog "selected_person_ids" hat (Mehrfachauswahl)
            if hasattr(dialog, 'selected_person_ids') and dialog.selected_person_ids:
                for pid in dialog.selected_person_ids:
                    self.db_manager.assign_person_to_shift(pid, self.current_shift_id)
            
            # Fallback für Einzelauswahl (falls wir mal zurückwechseln)
            elif hasattr(dialog, 'selected_person_id') and dialog.selected_person_id:
                 self.db_manager.assign_person_to_shift(dialog.selected_person_id, self.current_shift_id)

            self.load_shift_data(self.current_shift_id)
            self.data_changed.emit()

    def _remove_helper(self):
        """Entfernt ALLE ausgewählten Helfer aus der Schicht."""
        # NEU: selectedItems() statt currentItem()
        selected_items = self.assigned_list.selectedItems()
        
        if not selected_items or self.current_shift_id is None:
            return

        # Namen sammeln für die Frage
        names = [item.text() for item in selected_items]
        if len(names) == 1:
            msg = f"Möchten Sie '{names[0]}' wirklich aus dieser Schicht entfernen?"
        else:
            msg = f"Möchten Sie wirklich {len(names)} Personen aus dieser Schicht entfernen?"
        ...
        if reply == QMessageBox.Yes:
            # NEU: Schleife über alle markierten Items
