...
class RankingWidget(QWidget):
    ...
    def load_data(self):
        ...
        if self.rb_ranking.isChecked():
            self.ranking_options.setVisible(True)
            self.hours_options.setVisible(False)
            self._load_ranking_data()
        elif self.rb_hours.isChecked():
            self.ranking_options.setVisible(False)
    ...
    def _load_ranking_data(self):
        """Lädt die Score-Daten und füllt die Tabelle."""
        self.table.clear()
        ...
        for row_idx, score_data in enumerate(scores):
            ...
            score_item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter) # GEÄNDERT
    ...
    def _load_hours_data(self):
        """Lädt die Stunden-Daten und füllt die Tabelle."""
        self.table.clear()
        self.table.setColumnCount(3)
        ...
        for row_idx, data in enumerate(hours_data):
            ...
            hours_item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter) # GEÄNDERT
    ...
    def _load_details_data(self):
        """Lädt die Detail-Matrix und füllt die Tabelle."""
        self.table.clear()
        self.table.setColumnCount(6)
        self.table.setHorizontalHeaderLabels([
            "Name", "Geleistete Stunden", "Erledigt", "Als Vertreter", "Entschuldigt", "Nicht Erschienen"
        ])
        ...
        for row_idx, data in enumerate(details_data):
            ...
            # Alle numerischen Spalten rechtsbündig ausrichten
        ...
        sum_items = [
            f"{total_hours:.2f}", str(total_done), str(total_substitute),
            str(total_excused), str(total_absent)
        ]
        for col_idx, text in enumerate(sum_items, start=1):
            item = QTableWidgetItem(text)
            item.setFont(bold_font)
            item.setBackground(gray_background)
            item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter) # GEÄNDERT
    ...
    def export_summary(self):
        """Exportiert die aktuelle Ansicht (Stunden oder Details) als XLSX."""
        time_filter_text = self.hours_filter_combo.currentText()
        time_filter_db = (
            "current_year" if time_filter_text == "Aktuelles Jahr" else "all"
        )

        if self.rb_hours.isChecked():
            export_data = self.db_manager.get_hours_and_duties_summary(
                time_filter=time_filter_db
            )
            default_filename = (
                f"Stundenuebersicht_{time_filter_text.replace(' ', '_')}.xlsx"
            )
            export_func = Exporter.export_hours_summary_to_xlsx
        elif self.rb_details.isChecked():
            export_data = self.db_manager.get_detailed_member_summary(
                time_filter=time_filter_db
            )
            default_filename = (
                f"Detail-Matrix_{time_filter_text.replace(' ', '_')}.xlsx"
            )
            export_func = Exporter.export_detailed_summary_to_xlsx
        else:
            return

        if not export_data:
            QMessageBox.information(
                self, "Keine Daten", "Es gibt keine Daten zum Exportieren."
            )
            return
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Exportieren", default_filename, "Excel-Datei (*.xlsx)"
        )
        if not file_path:
            return

        self.settings.set_last_export_path(os.path.dirname(file_path))
        success = export_func(export_data, file_path, time_filter_text)

        if success:
            QMessageBox.information(
                self,
                "Export erfolgreich",
                f"Die Übersicht wurde erfolgreich nach\n{file_path}\nexportiert.",
            )
        else:
            QMessageBox.critical(
                self,
                "Export fehlgeschlagen",
                "Die Übersicht konnte nicht exportiert werden.",
            )
